...
PDFS = [
    ...
    ("[국문] 질병관리청 바로알기 건강한성인", DATA / "knsn/[질병관리본부] 일반인을 위한 만성콩팥병 바로알기_건강한성인편.pdf"),
...
def main() -> None:
    ...
    for sub in MD_SUBDIRS:
        ...
        for m in mds:
            ...
            if h[1] + h[2] + h[3] == 0:
                nohdr += 1
        print(f"  {sub:14s}: {len(mds):3d}개 | 합계 #={tot[1]} ##={tot[2]} ###={tot[3]} | 헤더0 파일={nohdr}/{len(mds)}")
        for k in grand:
            grand[k] += tot[k]
        grand_files += len(mds)
        grand_nohdr += nohdr
    print("-" * 64)
    print(f"  {'합계':14s}: {grand_files:3d}개 | #={grand[1]} ##={grand[2]} ###={grand[3]} | 헤더0 파일={grand_nohdr}/{grand_files}")
    print()
