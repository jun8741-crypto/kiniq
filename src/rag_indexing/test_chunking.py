...
def test_emit_parent_children_integrity():
    ...
    ck._emit_parent_children(
        parents=parents, children=children, text=long_text,
        source="TEST", doc_type="clinical", language="ko",
        h1="Chapter", h2="Section", parent_counter=[0],
    )
...
def test_emit_skips_tiny_text():
    parents, children = [], []
    ck._emit_parent_children(
        parents=parents, children=children, text="짧음",
        source="T", doc_type="clinical", language="ko",
        h1="", h2="", parent_counter=[0],
    )
