...
def guard_node(state: RAGState) -> dict:
    if EMERGENCY.search(_q(state)):
        ...
        return {
            "blocked": "긴급한 상황이라면 즉시 119, 자살예방상담 1393으로 연락하세요. "
...
def build_graph():
    ...
    for name, fn in [
        ...
        ("hallucination", hallucination_node),
        ("answer_grade", answer_node),
        ("post_guard", post_guard_node),
    ]:
        b.add_node(name, fn)
    b.add_edge(START, "guard")
    b.add_conditional_edges(
        "guard", lambda s: "blocked" if s["blocked"] else "retrieve", {"blocked": END, "retrieve": "retrieve"}
    )
