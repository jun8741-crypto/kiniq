...
#### 토끼 1단계 (rabbit_stage1.png)
```
신장 건강 앱 마스코트용 아기 토끼를 그려줘.
- 순백색(#FFFFFF) 몸 + 분홍색(#FBB6CE) 귀 안쪽
- 길고 둥근 귀 두 개, 큰 동그란 검은 눈
- 분홍색 코, 작은 앞니 2개 살짝 보임
- 분홍 볼터치, 두 손을 가슴 가까이 모은 포즈
- 카와이 산리오 스타일, 투명 배경, 1024x1024 PNG
```
...
- 가슴에 황금 별 메달
- 한 손에 황금 당근 (잎도 황금)
- 황금색 후광이 감쌈, 주변에 별과 반짝이
- 우아하고 위엄있지만 귀여운 표정
- 카와이 궁극 형태, 투명 배경, 1024x1024 PNG
```

#### 판다 1단계 (panda_stage1.png)
```
신장 건강 앱 마스코트용 아기 판다를 그려줘.
- 흰색(#F9FAFB) 본체 + 검정(#1F2937) 귀·팔·발
- 특징적인 검은 다크서클(눈테), 동그란 큰 눈
- 검은 코, 작은 미소
- 분홍 볼터치, 동글동글 통통한 몸
- 카와이 산리오 스타일, 투명 배경, 1024x1024 PNG
```
...
EggWidget 캐릭터 뒤에 표시되는 배경. 사용자의 챌린지 숙련도에 따라 자동 전환.

**저장 위치**: `frontend/ckd-care-app/public/backgrounds/`
**파일명**: `bg_proficiency1.png` ~ `bg_proficiency4.png` (또는 .svg)
**권장 크기**: 220×220 또는 정사각 PNG

#### 1단계 — 잔디 (입문, bg_proficiency1.png)
```
운동 입문자용 평화로운 잔디밭 배경을 그려줘.
- 연두색·초록색 그라데이션 풀밭
- 작은 풀잎·들꽃 (노랑·분홍)
- 하늘은 옅은 하늘색, 작은 햇살
- 캐릭터 마스코트가 그 위에 올라설 거니까 중앙은 비워주고 가장자리에 디테일
- 정사각형, 부드럽고 따뜻한 톤
```

#### 2단계 — 산스장 (초보, bg_proficiency2.png)
```
초보 등산용 산악 배경을 그려줘.
- 멀리 보이는 회청색 산봉우리, 일부 눈 덮임
- 가까이 짙은 녹색 산자락
- 흰 구름과 작은 해
- 중앙은 캐릭터 자리니까 비워두고 가장자리에 산·구름 배치
- 정사각형, 청량한 톤
```

#### 3단계 — 헬스장 (중급, bg_proficiency3.png)
```
실내 헬스장 배경을 그려줘.
- 어두운 회청색 톤 (#1E293B → #475569 그라데이션)
- 좌우에 덤벨, 천장 LED 조명
- 격자 마루 바닥
- 중앙은 비워둠 (캐릭터 자리)
- 정사각형, 진지하고 강한 톤
...
#### 4단계 — UFC 옥타곤 / 지옥 (숙련, bg_proficiency4.png)
```
UFC 옥타곤 케이지 + 지옥불 배경을 그려줘.
- 빨강·주황 그라데이션 바닥 (용암 느낌)
- 검은 옥타곤 격자 윤곽
- 좌우 바닥에 작은 불꽃
- 중앙은 캐릭터 자리니까 비워두고 가장자리만 강렬하게
- 정사각형, 강렬한 빨강·검정·황금 톤
```
