#!/usr/bin/env bash
# 발표 시연 직전에 demo 계정 상태를 빠르게 조작하는 헬퍼 (v3 — 3단계 진화 시스템).
#
# 사용:
...
case "${1:-}" in
    near-hatch)
        ...
        USER_ID=$(get_user_id)
        psql_exec "UPDATE user_eggs SET progress_checkins=9, current_stage=0, species=NULL, character_name=NULL, hatched_at=NULL, goal_70_alerted=false, goal_90_alerted=false, stage_25_bonus_paid=false, stage_50_bonus_paid=false, stage_75_bonus_paid=false, stage_100_bonus_paid=false WHERE user_id=$USER_ID AND current_stage < 3;"
        echo "✓ 완료. 시연 시 체크인 1번 = 부화 (종 추첨 + 이름 생성 + 컨페티)"
        ;;
    near-stage-2)
        echo "🥚 알 39회 설정 (다음 체크인 = 2단계 진화 +400pt)"
        USER_ID=$(get_user_id)
        psql_exec "UPDATE user_eggs SET progress_checkins=39, current_stage=1, stage_25_bonus_paid=true, stage_50_bonus_paid=false, stage_75_bonus_paid=false, stage_100_bonus_paid=false, goal_70_alerted=false, goal_90_alerted=false WHERE user_id=$USER_ID AND current_stage < 3;"
        echo "✓ 완료. 캐릭터가 이미 부화한 상태에서 진화 시연 가능"
        ;;
    near-stage-3)
        echo "🥚 알 99회 설정 (다음 체크인 = 3단계 최종 진화 +750pt, 완전체)"
        USER_ID=$(get_user_id)
        psql_exec "UPDATE user_eggs SET progress_checkins=99, current_stage=2, stage_25_bonus_paid=true, stage_50_bonus_paid=true, stage_75_bonus_paid=false, stage_100_bonus_paid=false, goal_70_alerted=false, goal_90_alerted=true WHERE user_id=$USER_ID AND current_stage < 3;"
        echo "✓ 완료. 시연 시 완전체 진화 모달 + 노란 강조"
        ;;
    near-final-alert)
        echo "🔔 알 89회 설정 (다음 체크인 = '최종 진화 임박' 알림 발동)"
        USER_ID=$(get_user_id)
        psql_exec "UPDATE user_eggs SET progress_checkins=89, current_stage=2, stage_25_bonus_paid=true, stage_50_bonus_paid=true, stage_75_bonus_paid=false, stage_100_bonus_paid=false, goal_70_alerted=false, goal_90_alerted=false WHERE user_id=$USER_ID AND current_stage < 3;"
    ...
    *)
        cat <<EOF
demo 계정 시연 헬퍼 (v3 — 3단계 진화 시스템)

진화 임계: 부화 10 / 2단계 40 / 완전체 100
...
진화 명령:
  ...
  near-stage-2      39회 → 다음 체크인 = 2단계 진화 +400pt
  near-stage-3      99회 → 다음 체크인 = 3단계 최종 진화 +750pt (완전체)
  near-final-alert  89회 → 다음 체크인 = 최종 진화 임박 알림 (Goal Gradient)
